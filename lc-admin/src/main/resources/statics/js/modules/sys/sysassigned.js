            ...
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    confirm('确定要分配到该宿舍吗？', function(){
                        $.ajax({
                            type: "POST",
                            url: baseURL + "sys/sysstudent/updateRoom?stuIds="+stuIds+"&roomCode="+stuRoomCode+"&roomName="+stuRoomName,
                            contentType: "application/json",
                            data: {'stuIds':stuIds,'roomCode':stuRoomCode,'roomName':stuRoomName},
                            success: function(result){
                                if(result.code == 0){
                                    alert('操作成功', function(index){
                                        $("#jqGrid").trigger("reloadGrid");
